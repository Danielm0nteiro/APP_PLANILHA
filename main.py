    ...
    if request.method == 'POST':
        ...
        if uploaded_file.filename != '':
            ...
            if file_ext not in ['.xls', '.xlsx', '.csv']:
                flash('Por favor, envie um arquivo Excel (.xls, .xlsx) ou CSV (.csv).')
            ...
            try:
                ...
                processar_planilha(file_path, file_ext, numeros_para_remover, coluna_contato, max_linhas_por_planilha)
...
def processar_planilha(file_path, file_ext, numeros_para_remover, coluna_contato, max_linhas_por_planilha):
    try:
        if file_ext == '.csv':
    ...
    coluna_contato = coluna_contato.strip().lower()

    # Listar colunas disponíveis
    print("Colunas disponíveis na planilha:", df.columns.tolist())

    if coluna_contato not in df.columns:
        colunas_disponiveis = ', '.join(df.columns.tolist())
        raise ValueError(f'A coluna "{coluna_contato}" não foi encontrada na planilha. Colunas disponíveis: {colunas_disponiveis}')
